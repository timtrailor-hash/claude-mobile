...
def bucket(n: int) -> str:
    if n < 100: return 'size<100'
    if n < 1024: return 'size<1k'
    if n < 10240: return 'size<10k'
    if n < 102400: return 'size<100k'
    return 'size>=100k'
...
def fingerprint_one(base: str, ep: str) -> dict:
    ...
    try:
        ...
        return {'endpoint': ep, 'method': 'GET', 'status': 'ERR', 'content_type': '', 'shape': f'unreachable: {e}'}
    return {
        'endpoint': ep,
        'method': 'GET',
        'status': str(status),
